...
yargs
  ...
  .command({
    ...
    handler: async (args) => {
      ...
      const staticsTemplate = fs.readFileSync(path.join(rootDir, "src", "statics.template.ts"), "utf8");
      const subgraphTemplate = fs.readFileSync(path.join(rootDir, "subgraph.template.yaml"), "utf8");
