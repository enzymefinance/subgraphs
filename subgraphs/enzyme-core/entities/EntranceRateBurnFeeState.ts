...
import { EntranceRateBurnFeeState, Fee, Vault } from '../generated/schema';
...
export function entranceRateBurnFeeStateId(vault: Vault, event: ethereum.Event): string {
  return vault.id + '/' + event.block.timestamp.toString() + '/feeState/entranceRateBurn';
}
...
export function createEntranceRateBurnFeeState(
  ...
  let feeState = new EntranceRateBurnFeeState(entranceRateBurnFeeStateId(vault, event));
...
export function ensureEntranceRateBurnFeeState(
  ...
  let entranceRateBurnFeeState = EntranceRateBurnFeeState.load(
    entranceRateBurnFeeStateId(vault, event),
