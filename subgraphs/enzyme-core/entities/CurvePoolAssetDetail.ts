...
export function checkCurvePoolAssetDetail(derivative: Asset, derivativePriceFeedAddress: Address): void {
  ...
  details.token0 = coinsValue[0].equals(ETH) ? wethTokenAddress.toHex() : coinsValue[0].toHex();
  details.token1 = coinsValue[1].equals(ETH) ? wethTokenAddress.toHex() : coinsValue[1].toHex();
  if (nCoinsValue[0].toI32() == 3) {
    details.token2 = coinsValue[2].equals(ETH) ? wethTokenAddress.toHex() : coinsValue[2].toHex();
  }
  details.save();
