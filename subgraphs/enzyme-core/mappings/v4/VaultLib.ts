...
import { useMysoV3OptionWritingPosition } from '../../entities/MysoV3OptionWritingPosition';
...
export function handleExternalPositionAdded(event: ExternalPositionAdded): void {
  ...
  if (type.label == 'MYSO_V3_OPTION_WRITING') {
    let ap = useMysoV3OptionWritingPosition(event.params.externalPosition.toHex());
    ap.active = true;
    ap.save();
  }
